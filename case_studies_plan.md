...
| `scripts/tests/test_attraction_power.py` | **article** → `experiments/03_forces/` | phrase autograd 7.3(i) |
| `scripts/approximations_finetune_run.py` | **article** → `experiments/03_forces/` | Table 4 (cross-Procrustes) |
...
| `scripts/tests/test_log_volume*.py` | **vitrine** (démo objectif composite RV − τ log Z) ou archive | option |
...
- (ii) relancer le finetune aux hyperparamètres canoniques (perplexité/k=30, γ=0.5, n=2000) et
  produire la table 4×2 des Procrustes croisés — vérifier que la conclusion (fw_tSNE ≈ fw_UMAP ≪
  refs) tient à ces réglages ; si l'écart se resserre, sweep léger en vitrine pour montrer la
  robustesse.
- *Rangement* : `experiments/03_forces/` ; scripts scatter/figures du finetune → `showcase/` ;
  trancher le sort de `test_log_volume*.py` (vitrine « objectif composite » ou archive).
...
- §7.3 : les 4 Procrustes croisés (ex-0.124/0.184/0.294/0.362 à hp=15) → valeurs à hp=30.
