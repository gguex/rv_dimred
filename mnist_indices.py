...
mnist_images_tensor = torch.tensor(mnist_images, dtype=torch.float32).to(device)
...
n_neighbors = 12
perplexity = 30
gauss_params, _ = binary_search_rbf_params(mnist_images, target_perplexity=perplexity)

# All input kernel functions to test
K_lin_in = compute_linear_kernel_torch(
    mnist_images_tensor, param=None, weights=weights, device=device
)
K_geo_in_cpu = compute_geodesic_kernel(
    mnist_images, param=n_neighbors, weights=weights.to("cpu").numpy()
)
K_geo_in = torch.tensor(K_geo_in_cpu, dtype=torch.float32).to(device)
K_lle_in = compute_lle_kernel_torch(
    mnist_images_tensor, param=n_neighbors, weights=weights, device=device
)
K_gauss_in = compute_gaussP_kernel_torch(
    mnist_images_tensor, param=gauss_params, weights=weights, device=device
)
K_topo_in = compute_fuzzy_topo_kernel_torch(
    mnist_images_tensor, param=n_neighbors, weights=weights, device=device
)
...
Y_lle = LocallyLinearEmbedding(n_components=2, n_neighbors=n_neighbors).fit_transform(
    mnist_images
)
Y_lap = SpectralEmbedding(n_components=2, n_neighbors=n_neighbors).fit_transform(
    mnist_images
)
...
Y_pca = torch.tensor(
    PCA(n_components=2).fit_transform(mnist_images_tensor.cpu().numpy())
)
...
for i, combination in enumerate(combinations):
    ...
    Y_opt_torch, RV_final_torch = rv_ascent_torch(
        K_in,
    ...
    thrust_row = [
        trustworthiness(Y_opt.numpy(), Y_comp, n_neighbors=n_neighbors)
        for Y_comp in Y_comp_list
    ]
    RV_matrix[i, :] = RV_row
