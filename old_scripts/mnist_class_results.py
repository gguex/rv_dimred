...
alpha = 0.3
K_lin_mix_in = alpha * K_lin_in / N_lin + (1- alpha) * K_Z / N_Z
K_poly_mix_in = alpha * K_poly_in / N_poly + (1- alpha) * K_Z / N_Z
K_rbf_mix_in = alpha * K_rbf_in / N_rbf + (1- alpha) * K_Z / N_Z
K_t_mix_in = alpha * K_t_in / N_t + (1- alpha) * K_Z / N_Z   

# the 4 outputs for all the input kernels
kernels_in = [K_lin_mix_in, K_poly_mix_in, K_rbf_mix_in, K_t_mix_in]
...
for i in range(4):
    for j in range(4):
        ...
        ax.set_yticks([])
plt.tight_layout()
plt.savefig("results/mnist/rv_mnist_cls.png", dpi=300)
