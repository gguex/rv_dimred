...
Y_opt_torch, RV_final_torch = rv_descent_torch(K_in, compute_cosine_kernel_torch, 
                                               param=None,
...
scatter = plt.scatter(Y_opt[:,0], Y_opt[:,1], c=mnist_labels, cmap='tab10', 
                      s=10)
plt.title(f"MNIST Spherical Gaussian RV (Perplexity={perplexity})")
plt.xlabel("Dimension 1")
plt.ylabel("Dimension 2")
plt.colorbar(scatter, ticks=range(10), label='Digit Label')
plt.grid(True)
plt.savefig("results/mnist/mnist_spherical_rv.png", dpi=300)
plt.show()  