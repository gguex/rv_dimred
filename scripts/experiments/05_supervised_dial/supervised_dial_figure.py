"""Plot metrics for the supervised kernel interpolation.
...
train and test curves against the double dial beta, with beta = 0.5 marked as an
intermediate point. Endpoint labels describe the actual hollow-RV objectives.
...
DATASET_LABELS = {"mnist": "MNIST", "singlecell": "Single-cell"}
...
def main() -> None:
    ...
    for col, ds_name in enumerate(DATASETS):
        for row, (metric, ylabel) in enumerate(METRICS):
            ...
            if row == 0:
                ax.set_title(DATASET_LABELS[ds_name], fontsize=12)
            if col == 0:
                ax.set_ylabel(ylabel)
            if row == 1:
                ax.set_xlabel(r"dial $\beta$  (0 = affinity target, 1 = class target)")
            ax.grid(alpha=0.25)
